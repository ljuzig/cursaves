"""Ignore stale Composer pane tombstones (v0.9.15).

Cursor stores leftovers as either JSON text ``"null"`` or a true
SQLite NULL in ``composerData.value``. Both are ``NULL_TOMBSTONE``.
"""
...
from cursor_saves import cli, db, export, importer, paths, syncstate
...
def _put_null_composer(
    conn: sqlite3.Connection, cid: str, *, kind: str = "json"
) -> None:
    value = None if kind == "sql" else "null"
    conn.execute(
...
def _assert_null_kind(path, cid: str, kind: str) -> None:
    conn = sqlite3.connect(str(path))
    try:
        typeof, quoted, length = conn.execute(
            "SELECT typeof(value), quote(value), length(value) "
        ...
        conn.close()
    if kind == "sql":
        assert (typeof, quoted, length) == ("null", "NULL", None)
    else:
        assert typeof == "text"
        assert quoted == "'null'"
        assert length == 4
...
def _install_pane_tombstones(
    ...
    *,
    ...
    for cid in cids:
        cell_kind = (kinds or {}).get(cid, kind)
        _put_null_composer(gconn, cid, kind=cell_kind)
...
def test_strict_select_distinguishes_absent_sql_null_and_json_null(sync_env):
    gconn = _init_db(sync_env["global_db"])
    _put_null_composer(gconn, CID_A, kind="json")
    _put_null_composer(gconn, CID_B, kind="sql")
    gconn.commit()
    gconn.close()
    _assert_null_kind(sync_env["global_db"], CID_A, "json")
    _assert_null_kind(sync_env["global_db"], CID_B, "sql")

    with db.CursorDB(sync_env["global_db"]) as cdb:
        assert syncstate._strict_select_row(cdb, f"composerData:{CID_A}") == (
            True,
            None,
        )
        assert syncstate._strict_select_row(cdb, f"composerData:{CID_B}") == (
            True,
            None,
        )
        assert syncstate._strict_select_row(cdb, f"composerData:{CID_C}") == (
            False,
            None,
        )

    with syncstate.SyncReadSession() as session:
        assert session.composer_row_is_json_null(CID_A)
        assert session.composer_row_is_json_null(CID_B)
        assert not session.composer_row_is_json_null(CID_C)
        assert (
        ...
        assert syncstate.classify_local_payload(None) == (
            syncstate.LocalPresence.INVALID
        )


@pytest.mark.parametrize("kind", ["json", "sql"])
def test_pane_null_tombstone_absent_from_plan_and_not_unknown(sync_env, kind):
    active = _active(CID_A)
    _commit_env(sync_env, [active], [active])
    _install_pane_tombstones(sync_env, [CID_B], kind=kind)
    _assert_null_kind(sync_env["global_db"], CID_B, kind)
...
@pytest.mark.parametrize("kind", ["json", "sql"])
def test_global_hidden_and_workspace_pane_ignored_once(sync_env, kind):
    active = _active(CID_A)
    _commit_env(sync_env, [active], [active])
    _install_pane_tombstones(
        sync_env, [CID_B], also_global_hidden=True, kind=kind
    )
...
@pytest.mark.parametrize("kind", ["json", "sql"])
def test_null_tombstone_with_snapshot_is_importable(sync_env, kind):
    remote = _conversation([_msg(1, "backed-up")], composer_id=CID_B, name="Remote")
    _write_workspace(sync_env["ws_dir"], [])
    gconn = _init_db(sync_env["global_db"])
    _put_null_composer(gconn, CID_B, kind=kind)
    gconn.commit()
    gconn.close()
    _install_pane_tombstones(sync_env, [CID_B], kind=kind)
...
@pytest.mark.parametrize("kind", ["json", "sql"])
def test_null_tombstone_with_typed_row_is_diagnosed(sync_env, monkeypatch, kind):
...
def test_task_pane_null_tombstone_never_typed(sync_env, monkeypatch):
    ...
    gconn = sqlite3.connect(str(sync_env["global_db"]))
    _create_typed_table(gconn)
    _put_null_composer(gconn, task_id, kind="sql")
    gconn.commit()
    gconn.close()
    _install_pane_tombstones(sync_env, [task_id], kind="sql")
...
def test_active_plus_pane_tombstones_status_and_sync(
    ...
    kinds = {
        cid: ("json" if i < 4 else "sql") for i, cid in enumerate(TOMBSTONES)
    }
    _install_pane_tombstones(
        sync_env, TOMBSTONES, also_global_hidden=True, kinds=kinds
    )
    for cid, kind in kinds.items():
        _assert_null_kind(sync_env["global_db"], cid, kind)
