...
def _mixed_workspace_env(sync_env, *, with_typed_table: bool = True, typed_rows: bool = False):
    snaps = [_active(cid, name) for cid, name in SAMPLE_CHATS]
    ...
    _write_legacy_json(
        gconn,
        [_legacy_header(cid, name) for cid, name in SAMPLE_CHATS],
    )
    if typed_rows:
...
def test_repair_does_not_modify_composer_or_bubbles(sync_env, monkeypatch):
    _restore_real_headers(monkeypatch)
    _mixed_workspace_env(sync_env)
    before = {cid: _content_blob(sync_env["global_db"], cid) for cid, _ in SAMPLE_CHATS}
    plan = _plan(sync_env, monkeypatch)
    assert importer.repair_typed_registrations(plan) == 4
    after = {cid: _content_blob(sync_env["global_db"], cid) for cid, _ in SAMPLE_CHATS}
    assert before == after
...
def test_healthy_noop_sync_does_not_write(sync_env, monkeypatch):
    _restore_real_headers(monkeypatch)
    _mixed_workspace_env(sync_env, typed_rows=True)
    _use_workspaces(monkeypatch, [_ws(sync_env["ws_dir"], PROJECT_PATH)])
    _backend(monkeypatch)
    monkeypatch.setattr(
...
def test_repair_via_sync(sync_env, monkeypatch, capsys):
    _restore_real_headers(monkeypatch)
    snaps = _mixed_workspace_env(sync_env)
    before = {cid: _content_blob(sync_env["global_db"], cid) for cid, _ in SAMPLE_CHATS}
    before_digests = {
        cid: syncstate.snapshot_semantic_digest(snap) for (cid, _), snap in zip(SAMPLE_CHATS, snaps)
    }
    _use_workspaces(monkeypatch, [_ws(sync_env["ws_dir"], PROJECT_PATH)])
    _backend(monkeypatch)
    ...
    after = {cid: _content_blob(sync_env["global_db"], cid) for cid, _ in SAMPLE_CHATS}
    assert before == after
    for (cid, _), snap in zip(SAMPLE_CHATS, snaps):
...
def test_archived_legacy_only_stays_archived_after_typed_migration(
    ...
    _mixed_workspace_env(sync_env)
    gconn = sqlite3.connect(str(sync_env["global_db"]))
    headers = [_legacy_header(cid, name) for cid, name in SAMPLE_CHATS]
...
def test_subagent_is_not_migrated_as_top_level(sync_env, monkeypatch):
    ...
    _write_workspace(
        sync_env["ws_dir"],
        [_active(cid, name) for cid, name in SAMPLE_CHATS] + [extra],
    )
    _use_workspaces(monkeypatch, [_ws(sync_env["ws_dir"], PROJECT_PATH)])
...
def test_migrate_skips_ambiguous_cid_claimed_by_multiple_workspaces(
    ...
    _mixed_workspace_env(sync_env)
    ws_b = sync_env["tmp"] / "cursor" / "workspaceStorage" / WS_HASH_B
    _write_workspace(ws_b, [_active(CID_A, "Chat A")])
...
def _local_only_fixture(sync_env, *, typed_rows: bool = False):
    snaps = [_active(cid, name) for cid, name in SAMPLE_CHATS]
...
def test_empty_snapshot_index_registered_is_true_noop(sync_env, monkeypatch):
    _restore_real_headers(monkeypatch)
    _local_only_fixture(sync_env, typed_rows=True)
    _use_workspaces(monkeypatch, [_ws(sync_env["ws_dir"], PROJECT_PATH)])
    _backend(monkeypatch)
    syncstate.reset_op_counts()
    cli.cmd_sync(type("Args", (), {"force": False})())
...
def test_task_like_selected_pane_only_is_not_typed(sync_env, monkeypatch):
    ...
    _write_workspace(
        sync_env["ws_dir"],
        [_active(cid, name) for cid, name in SAMPLE_CHATS],
    )
    pane_key = "workbench.panel.composerChatViewPane.main"
    with db.CursorDB(sync_env["ws_dir"] / "state.vscdb") as cdb:
        data = cdb.get_json("composer.composerData", table="ItemTable") or {}
