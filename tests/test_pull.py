...
def test_ssh_other_hosts_never_enter_pull_plan(sync_env):
    ...
    other_b = _conversation(
        ...
        sourceHost="host-b",
        sourceProjectPath=ssh_path,
        projectIdentifier="ssh-host-b-home-user-project",
    )
    other_c = _conversation(
        ...
        sourceHost="host-c",
        sourceProjectPath=ssh_path,
        projectIdentifier="ssh-host-c-home-user-project",
    )
    _write_workspace(sync_env["ws_dir"], [])
    _write_snapshot_file(sync_env["snaps"] / "ssh-host-a-home-user-project", mine)
    _write_snapshot_file(sync_env["snaps"] / "ssh-host-b-home-user-project", other_b)
    _write_snapshot_file(sync_env["snaps"] / "ssh-host-c-home-user-project", other_c)
    _init_db(sync_env["global_db"]).close()

    index = syncstate.SnapshotIndex.build_for_project("ssh-host-a-home-user-project")
    assert list(index.by_key) == [("ssh-host-a-home-user-project", CID_A)]
    workspace = {
        "path": ssh_path,
    ...
    assert plan.items[0].meta.get("sourceHost") == "host-a"
